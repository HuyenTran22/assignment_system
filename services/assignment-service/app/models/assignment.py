...
class Assignment(Base):
    ...
    # Relationships
    # Note: course relationship removed - Course is in course-service (microservices architecture)
    # We query Course separately when needed
    creator = relationship("User", foreign_keys=[created_by])
    files = relationship("AssignmentFile", back_populates="assignment", cascade="all, delete-orphan")
    submissions = relationship("Submission", back_populates="assignment", cascade="all, delete-orphan")
    # Note: Rubric relationship removed - Rubric model not yet implemented
